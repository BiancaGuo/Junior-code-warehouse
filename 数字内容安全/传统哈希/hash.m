...
% create hash ，使用java接口
x=java.security.MessageDigest.getInstance(meth); %Java 自带的加密类
x.update(inp); 
h=typecast(x.digest,'uint8'); %转换数据类型
h=dec2hex(h)'; 
if(size(h,1))==1 % remote possibility: all hash bytes 128, so pad: 
h=[repmat('0',[1 size(h,2)]);h]; %B = repmat(A,[m,n])，将矩阵 A 复制 m×n 块，即把 A 作为 B 的元素，B 由 m×n 个 A 平铺而成。size（h，2）：获取矩阵h的列数。将摘要值平铺到一行
end 
h=lower(h(:)'); %将hash值转换为小写字符串
clear x 
return