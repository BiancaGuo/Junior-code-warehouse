...
for i=5:100
    ...
    for m = 1:row
        for n = 1:col
             x = (m-1)*blocksize+1; %blocksize=8
             y = (n-1)*blocksize+1;
             block_origin=cover_object(x:x+blocksize-1,y:y+blocksize-1);
             block_pending = pending_file(x:x+blocksize-1,y:y+blocksize-1);
             block_origin_dct = dct2(block_origin);%原始图像分块后做dct变换
             block_pending_dct = dct2(block_pending);%待测图像分块后做dct变换
             watermark_extract(k)=block_pending_dct(1,1)-block_origin_dct(1,1);%提取出的待测水印序
             block_origin = idct2(block_origin_dct);%逆dct
             block_pending = idct2(block_pending_dct);%逆dct
    ...
    watermark_file = fopen(strcat(path_watermark,'embed_jpeg_压缩_',num2str(i),'.txt'),'wt');%数据保存在你当前的文件夹下，文件名为Data.txt
