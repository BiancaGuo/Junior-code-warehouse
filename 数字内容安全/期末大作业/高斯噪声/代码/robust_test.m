...
% 计算psnr值
 ...
 % 计算提取水印相似度
    ...
    %含水印
    file_name_out=strcat(path_watermark,'embed_高斯噪声_',num2str(i),'.txt');
    watermark_file=strcat('watermark.txt');
    ...
    %不含水印
    file_name_out=strcat(path_watermark,'lena_高斯噪声_',num2str(i),'.txt');
...
title(strcat('高斯噪声 PSNR ~ Similarity图'));%标题
