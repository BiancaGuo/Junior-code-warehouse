...
% 计算psnr值
    path='.\jpeg压缩图片\'; 
    %嵌入dc
    file_name_out=strcat(path,'embed_jpeg_压缩_dc',num2str(i),'.jpg');
    compress_image=imread(file_name_out);
    image_psnr=psnr(cover_object,compress_image,Mc,Nc);
    PSNR(k) = image_psnr;%x轴
    %嵌入ac
    file_name_out2=strcat(path,'embed_jpeg_压缩_ac',num2str(i),'.jpg');
 ...
 % 计算提取水印相似度
    ...
    path_watermark='.\JPEG压缩水印提取\';
     %嵌入dc
    file_name_out=strcat(path_watermark,'embed_jpeg_压缩_dc',num2str(i),'.txt');
    ...
    %嵌入ac
    file_name_out=strcat(path_watermark,'embed_jpeg_压缩_ac',num2str(i),'.txt');
...
title(strcat('JPEG压缩 嵌入AC系数与嵌入DC系数比较图'));%标题
