...
public class TypeCheck extends Tree.Visitor {
	...
	private void checkCallExpr(Tree.CallExpr callExpr, Symbol f) {
		...
		if (f == null) {
			...
			if(callExpr.receiver==null&&!currentFunction.isStatik()&&!func.isStatik()){
				callExpr.receiver=new Tree.ThisExpr(callExpr.getLocation());
				callExpr.receiver.accept(this);
			}
			if(func.isStatik()) callExpr.receiver=null;
			for(Tree.Expr expr : callExpr.actuals) expr.accept(this);
			
			List<Type> funcArgList=func.getType().getArgList();
			int actualsSize=0;
			if(func.isStatik()) actualsSize=callExpr.actuals.size();
			else actualsSize=callExpr.actuals.size()+1;
			int argListSize=0;
			if(func.isStatik()) argListSize=funcArgList.size();
			else argListSize=funcArgList.size()-1;
			if(actualsSize!=funcArgList.size()){
				issueError(new BadArgCountError(callExpr.getLocation(), callExpr.method, argListSize, callExpr.actuals.size()));
				
			}
			else{
				int s=funcArgList.size()-argListSize;
				for(int i=0;i<argListSize;i++){
					if(!callExpr.actuals.get(i).type.compatible(funcArgList.get(i+s))){
						issueError(new BadArgTypeError(callExpr.actuals.get(i).getLocation(), i+1,callExpr.actuals.get(i).type.toString(), funcArgList.get(i).toString()));
					}
				}
			}
		}
	...
	public void visitNewArray(Tree.NewArray newArrayExpr) {
		...
		if(newArrayExpr.elementType.type.equal(BaseType.VOID)||newArrayExpr.elementType.type.equal(BaseType.NULL)){
	...
	public void visitThisExpr(Tree.ThisExpr thisExpr) {
		...
		if(currentFunction.isStatik()){
			...
			thisExpr.type=((ClassScope)table.lookForScope(Kind.CLASS)).getOwner().getType();
		}
	}
	...
	public void visitTypeTest(Tree.TypeTest instanceofExpr) {
		...
		{
			issueError(new NotClassError(instanceofExpr.instance.type.toString(),
					instanceofExpr.getLocation()));
		}
	...
	public void visitIdent(Tree.Ident ident) {
		if (ident.owner == null) {
			...
			if (v == null) {
				...
				if(v.getScope().isClassScope()&&currentFunction.isStatik()){
				...
				}else if(ident.symbol.isMemberVar()){
	...
	public void visitAssign(Tree.Assign assign) {
		...
		if((!assign.expr.type.compatible(assign.left.type)||assign.left.type.isFuncType())&&(!assign.left.type.equal(BaseType.ERROR)||!assign.left.type.equal(BaseType.NULL))){
	...
	public void visitBreak(Tree.Break breakStmt) {
		if (breaks.empty()/* TODO: REPLACE THIS WITH A CORRECT CONDITION */) {
	...
	public void visitTypeArray(Tree.TypeArray typeArray) {
		...
		} else if (typeArray.elementType.type.equal(BaseType.VOID)||typeArray.elementType.type.equal(BaseType.NULL)) {
	...
	private Type checkBinaryOp(Tree.Expr left, Tree.Expr right, int op, Location location) {
		...
		case Tree.AND:
		case Tree.OR:
			compatible=left.type.equal(BaseType.BOOL)&&right.type.equal(BaseType.BOOL);
			returnType=BaseType.BOOL;
			break;
		// TODO: 为上面每个case添加代码
		default:
