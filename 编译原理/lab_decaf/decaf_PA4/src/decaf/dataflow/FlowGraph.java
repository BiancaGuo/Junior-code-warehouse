...
public class FlowGraph implements Iterable<BasicBlock> {
	...
	public void analyzeLiveness() {
		...
		while(changed){
			changed = false;
			for (int i = n - 1; i >= 0; i --) {
				basicBlock = bbs.get(i);
				int Num = basicBlock.next.length;
				for (int j = 0; j < Num; ++j)
					basicBlock.liveOut.addAll(bbs.get(basicBlock.next[j]).liveIn);				
				newLiveIn.clear();
				tempSet.clear();
				newLiveIn.addAll(basicBlock.liveUse);
				tempSet.addAll(basicBlock.liveOut);
				tempSet.removeAll(basicBlock.def);
				newLiveIn.addAll(tempSet);
				if (!newLiveIn.equals(basicBlock.liveIn)){
					changed = true;
					basicBlock.liveIn.clear();
					basicBlock.liveIn.addAll(newLiveIn);
				}
			} 
		} 
	}
